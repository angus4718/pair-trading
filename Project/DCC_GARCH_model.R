...
# Pairs information ------
pair_description = read.csv("output/pair_df.csv", row.names=1)
...
library(xts)
train.pair_spread = read.csv("output/train_spread.csv", row.names=1)
train.pair_spread = as.xts(train.pair_spread) # Convert spreads to time series
train.half_life = read.csv("output/train_half_life.csv", row.names=1)
...
test.pair_spread = read.csv("output/test_pair_spread.csv", row.names=1)
test.pair_spread = as.xts(test.pair_spread)
test.half_life = read.csv("output/test_half_life.csv", row.names=1)
for (col in 1:pairs.no){
...
# Write training csv -------
write.zoo(as.zoo(test.spread_scaled), file="output/test_spread_scaled.csv", sep=",")
write.zoo(as.zoo(as.xts(test.vol)), file="output/test_spread_vol.csv", sep=",")
write.zoo(as.zoo(test.z_score), file="output/test_z_score.csv", sep=",")
